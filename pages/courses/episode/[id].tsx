...
const EpisodePlayer = function () {
    ...
    const handleGetEpisodeTime = async () => {
        const res = await watchEpisodeService.getWatchTime(Number(episodeId));
        console.log(res)
        if (res.data !== null) {
                ...
                <Container className="d-flex flex-column align-items-center gap-3 pt-3">
                    ...
                    <div className={styles.episodeButtonDiv}>
                        <Button className={styles.episodeButton}
                            ...
                            <img src="/episodes/iconArrowLeft.svg"
                                alt="setaEsquerda"
                                className={styles.arrowImg} />
                        </Button>
                        <Button className={styles.episodeButton}
                            disabled={episodeOrder + 1 === course.episodes.length ? true : false}
                            onClick={handleNextEpisode}><img src="/episodes/iconArrowRight.svg"
                                alt="setaDireita"
                                className={styles.arrowImg} />
                        </Button>
                    </div>
